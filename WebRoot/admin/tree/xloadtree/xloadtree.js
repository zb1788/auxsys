...
WebFXLoadTree.prototype.reload = WebFXTreeItem.prototype.reload = 
...
WebFXLoadTree.prototype.checkedReload = 
WebFXLoadTreeItem.prototype.checkedReload = function () {
	try{
	if(!webFXTreeHandler.checked) return;
	for (var i in webFXTreeHandler.checked ) {
		webFXTreeHandler.checked[i].reload();
	}
	}catch(e){}
};
...
function _startLoadXmlTree(sSrc, jsNode) {
	...
	xmlDoc.onreadystatechange = function () {
		if (xmlDoc.readyState == 4) { //xml装载完毕后执行
	...
	window.setTimeout(function () {
        ...
        if(xmlhttp.readyState == 4){
        	xmlDoc = xmlParser(xmlhttp.responseText);
        	_xmlFileLoaded(xmlDoc, jsNode);
        } 
	}, 10);
}
