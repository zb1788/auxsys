...
Slider.prototype = {
  //容器对象，滑块
  initialize: function(container, bar,inputvalue, options,width) {
	this.Bar = $(bar);
	this.Lock=false;
	this.valuetype=true;
	...
	//实例化一个拖放对象，并限定范围
	this._drag = new Drag(this.Bar, { Limit: true,Lock:this.Lock, mxContainer: this.Container,
  ...
  KeyCtrl: function(e) {
	if(this.KeySpeed){
		...
		switch (e.keyCode) {
			case 37 ://左
				this.SetPercent(((parseInt($(this.Inputvalue).value)-1)- this.MinValue)/(this.MaxValue - this.MinValue));
				$(this.Inputvalue).value=parseInt($(this.Inputvalue).value)-1;
				break;
			case 38 ://上
				iTop -= iHeight; break;
			case 39 ://右
