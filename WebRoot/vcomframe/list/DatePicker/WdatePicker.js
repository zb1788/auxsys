...
(function(){
  ...
  if($.$crossFrame){
    try{
    if(!T.parent.managelayout){
    	if(T.parent.parent.managelayout)
      		T=T.parent;
      	else
      		T=T.parent.parent;
      }
  ...
  if(!T.$dp||L){
    $dp=Q({ff:G,ie:R,opera:Z,el:null,win:V,status:2,defMinDate:$.minDate,defMaxDate:$.maxDate,$:function(_){
        try{
          this.win[N];
        }
        catch($){
