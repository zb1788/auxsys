...
function grid(id,title,selfName,readerline,column,myData,splitpage,CheckboxSelectionModel,topbar,heigth,selectionnumber,type){
	...
	grid.prototype.changeSplitPage= function(sps) {
    	...
    	pe.All_Startelement.innerText = (this.sp.recordCount!=0?(this.sp.startRecord+1):0);
	...
	Ext.onReady(function(){
	    ...
	    // create the Grid
	    var splitbar=splitpage==null?null:new Ext.Toolbar({items: ["显示第 <span id=\"splitPage_All_Start_" + ___index + "\">"+(_this.sp.recordCount!=0?(_this.sp.startRecord+1):0)+"</span> 条到 <span id=\"splitPage_All_End_" + ___index + "\">"+(_this.sp.startRecord+_this.sp.pageSize>_this.sp.recordCount?_this.sp.recordCount:(_this.sp.startRecord+_this.sp.pageSize))+"</span> 条记录，一共 <span id=\"splitPage_All_Count_" + ___index + "\">"+_this.sp.recordCount+"</span> 条",'->',showpage()+""]});
	     //选择模式
	    ...
	    _this.grids = new Ext.grid.GridPanel({
	        ...
	        height:_this.type==true?_this.heigth:hi,
	        enableDragDrop: true,
	        autoScroll:true,
	        title:title,
			types:'grids',
